...
"""
CLI entry point for WorldTime
This module provides the command line interface for the WorldTime application,
...
console = Console()
FAV_FILE = Path.home() / ".worldtime_favorites.json"
...
def print_favorites(favs: List[str], meeting_time: Optional[datetime.datetime] = None):
    from .core import ZoneInfo
    if not favs:
        console.print("[red]No favorite cities set. Use 'worldtime add <city>' to add one.[/red]")
        console.print("[yellow]Use 'worldtime <city>' to search one and 'worldtime --help' for more info[/yellow]")
        return
    banner = Text("🌍 WORLD TIME FAVORITES 🌍", style="bold magenta on cyan", justify="center")
...
def print_compare(cities: List[str]):
    ...
    if not found:
        ...
        return
    table = Table(title="[bold magenta]WorldTime Compare[/bold magenta]", show_lines=True, box=ROUNDED, expand=False)
    table.add_column("Flag", style="bold", justify="center")
    table.add_column("City", style="bold cyan")
    table.add_column("Local Time", style="green")
    table.add_column("Phase", style="magenta")
...
def print_help():
    help_text = """
[bold cyan]WorldTime - Global Timezone Utility[/bold cyan]
...
def main():
    ...
    try:
        ...
        user = "user"
    console.print(f"[bold blue]{greeting}, {user}! Welcome to WorldTime 🌐[/bold blue]")
    ...
    if cmd == "meeting":
        ...
        if meeting_time is None:
            ...
            console.print("[yellow]See 'worldtime -h' for help.[/yellow]")
            return
    ...
    if city_info:
        ...
        console.print("[red]Invalid command or city not found. See 'worldtime -h' for help.[/red]")
